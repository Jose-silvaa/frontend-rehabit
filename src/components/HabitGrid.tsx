...
export function HabitGrid({ day }: { day: string }) {
  ...
  const toggleHabitCompletion = async (id: string) => {
    ...
    try {
      const response = await fetch("https://server-habit.onrender.com/habit/mark", {
        method: "POST",
        headers: {
  ...
  useEffect(() => {
    ...
    const fetchHabitsUser = async () => {
      ...
      try {
        const response = await fetch("https://server-habit.onrender.com/habit/all", {
          method: 'GET',
          headers: {
            'Content-Type': 'application/json'
          }
  ...
  const fetchDetailsRoutine = async (habitId: string) => {
    ...
    try {
      ...
      const response = await fetch("https://server-habit.onrender.com/users/details", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
  ...
  return (
    <div>
      {filteredHabits.length > 0 ? (
              ...
              <section key={habit.id}>
                <div className="bg-card text-card-foreground rounded-lg shadow-sm p-4 flex flex-col border border-gray-200 hover:border-primary transition-colors duration-200">
                  <div className="flex justify-between items-center mb-2 relative">
                    ...
                    <svg xmlns="http://www.w3.org/2000/svg" cursor="pointer" onClick={() => alert("Comming Soon")} className="h-5 w-5 shrink-0 opacity-60 absolute z-10 top-1/2 -translate-y-1/2 -left-3" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2">
                  ...
                  <div className="flex items-center justify-between mt-4">
                    ...
                    <Button onClick={() => toggleHabitCompletion(habit.id)} variant="ghost" size="sm" className={`w-6 h-6 p-0 border-2 relative rounded-sm  ${completedHabits.includes(habit.id) ? "bg-primary text-primary-foreground border-muted-foreground" : "bg-transparent border-muted-foreground"}`}>
        ...
        <p className="text-center text-gray-500 mt-4">No habits created.</p>
      )}
    </div>
  )
}
